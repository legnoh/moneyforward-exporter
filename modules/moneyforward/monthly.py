...
def set_monthly_metrics(driver, registry, config, metrics):
    period = driver.find_element_by_css_selector(config['css_selector']['title']).text.lstrip("(").rstrip(")")
    datas = driver.find_elements_by_css_selector(config['css_selector']['data'])
...
def set_latest_withdrawal_metrics(driver,registry,config,metrics):
    accounts = driver.find_elements_by_css_selector(config['css_selector']['accounts'])
    ...
    for account in accounts:
        try:
            name_raw = account.find_element_by_css_selector(config['css_selector']['name']).text

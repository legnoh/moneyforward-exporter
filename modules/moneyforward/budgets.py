...
def set_budget_metrics(driver, registry, config, metrics):
    ...
    period = driver.find_element_by_css_selector(config['css_selector']['period']).text
    ...
    budgets = driver.find_elements_by_css_selector(config['css_selector']['budgets'])
    for budget in budgets:
        progress = mf.format_balance(budget.find_element_by_css_selector(config['css_selector']['progress']).text)
        balance = mf.format_balance(budget.find_element_by_css_selector(config['css_selector']['balance']).text)
        ...
        if config['css_selector']['total_class_name'] in classes:
            ...
            name = budget.find_element_by_css_selector(config['css_selector']['name']).text
